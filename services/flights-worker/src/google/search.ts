...
import { buildGoogleFilters, encodeGoogleFilters } from "./encoding.js";
import { parseGoogleFlightsResponse, type ParsedGoogleFlight } from "./parsing.js";
...
export interface GoogleFlightOption {
  provider: "google";
  price: number;
  currency: string;
  priceAvailable: boolean;
  outbound: GoogleFlightLeg;
  inbound?: GoogleFlightLeg;
  bookingLink: string;
  isDayFlight: boolean;
}

export interface GoogleSearchResult {
  flights: GoogleFlightOption[];
  capturedAt: string;
}

export async function searchGoogleFlights(params: GoogleSearchParams): Promise<GoogleSearchResult> {
...
function mapToFlightOption(f: ParsedGoogleFlight): GoogleFlightOption {
  const outbound: GoogleFlightLeg = {
  ...
  const inbound: GoogleFlightLeg | undefined =
    f.segments.length > 1
      ? {
