  ...
  return (
    <div data-testid="itinerary-view" className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <span className="bg-gray-100 px-2 py-0.5 rounded-full font-mono">v{itinerary.version}</span>
      ...
      <div className="space-y-4">
        {itinerary.itineraryJson.map((day) => (
          <div key={day.day} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center gap-3 mb-3">
              <span className="bg-indigo-600 text-white text-sm font-bold w-8 h-8 rounded-full flex items-center justify-center">
                {day.day}
              </span>
              <div>
                <p className="font-semibold text-gray-900">{day.location}</p>
            ...
            {day.notes && (
              <p className="mt-2 text-xs text-amber-700 bg-amber-50 rounded p-2">{day.notes}</p>
            )}
          </div>
        ))}
      </div>
...
function FlightLegCard({
  ...
  return (
    <div className="bg-white rounded-lg border border-blue-200 p-3">
      <p className="text-xs font-semibold text-blue-600 mb-1">{label}</p>
