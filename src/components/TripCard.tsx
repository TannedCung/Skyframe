...
export function TripCard({ trip }: TripCardProps) {
  ...
  return (
    <Link href={`/trip/${trip.id}`} data-testid="trip-card">
      <div className="bg-white rounded-xl shadow-sm border border-line p-6 hover:shadow-md transition-shadow cursor-pointer">
        <div className="flex items-start justify-between mb-3">
          <h2 className="text-lg font-semibold text-ink-900 truncate">{trip.title}</h2>
          <span
        ...
        {trip.flexibilityDays > 0 && (
          <p className="text-ink-400 text-xs mt-1">±{trip.flexibilityDays} days flexibility</p>
        )}
      </div>
    </Link>
  );
